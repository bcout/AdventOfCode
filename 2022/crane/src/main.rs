...
fn part_1_main() {
    // Read the input into two separate lines vectors
    ...
    // Prepare the input so it's easier to read
    // All whitespace in the stacks input becomes a single hyphen, very easy to tell what column a letter is in
    for line in input.split("\n") {
        ...
        if reading_instructions {
            ...
            let t = re.replace_all(&fixed_line, "-");
            stacks_lines.push(t.to_string());
        }
    }

    // Create and populate an array of vectors for the stacks and their values
    const num_stacks: usize = 9;
    let mut stacks_array: [Vec<char>; num_stacks] = Default::default();
    let mut column;
    for line in stacks_lines {
        ...
        for c in line.chars() {
            if c == '-' {
                ...
                stacks_array[column].push(c);
            }
            column = column + 1;
        }
        //println!("{:?}", stacks_array[column]);
    }
    // We then have to reverse the stack (the column vector) because reading the values in puts them into their vector in the wrong order.
    for i in 0..num_stacks {
        stacks_array[i].reverse();
        //println!("{i}: {:?}", stacks_array[i]);
    }
    ...
    for instruction in instructions_lines {
        //println!("{instruction}");
        caps = re.captures(instruction).unwrap();
        ...
        println!("{} from {} -> {}", quantity, source, destination);
        for i in 0..quantity {
            let value = stacks_array[source].pop();
            if value == None {
                break;
            }
            stacks_array[destination].push(value.unwrap());
        }
    }

    for i in 0..num_stacks {
        print!("{:?}", stacks_array[i].pop().unwrap());
    }
    println!("\nFinished");
}
...
fn part_2_main() {
    ...
    for instruction in instructions_lines {
        ...
        for i in 0..quantity {
            ...
            let character = value.unwrap();
            crates_to_move.push(character);
            //stacks_array[destination].push(character);
        }
        crates_to_move.reverse();
        for c in crates_to_move {
            stacks_array[destination].push(c);
        }
    }
